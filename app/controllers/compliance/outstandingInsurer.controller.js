...
exports.exportOutstandingInsurer = async (req, res) => {
  try {
    ...
    reportData.forEach((row, i) => {
      ...
      dataRow.getCell(2).font = { name: "Tahoma", size: 11, bold: false }; // Insurer name not bold
      ...
      dataRow.eachCell((cell, colNumber) => {
        if (colNumber === 2) {
          cell.alignment = { vertical: "middle", horizontal: "left" }; // insurer column left aligned
          cell.value = String(cell.value).toUpperCase();
        } else {
    ...
    // Style the cell for the overall percent (column F)
    ...
    sheet.getColumn(2).width = 28; // INSURER
    for (let i = 3; i <= 6; i++) {
