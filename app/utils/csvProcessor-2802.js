...
class CsvProcessor {
  ...
  static async validateAndParseSftp(sftpClient, filePath, db) {
    return new Promise((resolve, reject) => {
      ...
      let rowNumber = 0; // Track row numbers for better error reporting
  ...
  static transformRow(row) {
    ...
    return {
      claimAction: row["Claim Action"] || row["claim_action"],
      merimenCaseId: row["Merimen Case ID"] || row["merimen_case_id"],
      claimType: row["Claim Type"] || row["claim_type"],
      insuranceClaimNo: row["Insurance Claim No"] || row["insurance_claim_no"],
      insuredName: row["Insured Name"] || row["insured_name"],
      insuredIdNo: row["Insured ID No."] || row["insured_id_no"],
      insuredTel: row["Insured Tel"] || row["insured_tel"],
      insuredVehRegNo: row["Insured VehRegNo"] || row["insured_veh_reg_no"],
      ...
      repairerName: row["Repairer Name"] || row["repairer_name"],
      handlingInsurer: row["Handling Insurer"] || row["handling_insurer"],
      claimantInsurer: row["Claimant Insurer"] || row["claimant_insurer"],
      ...
      adjAsgRemarks: row["Adj Asg. Remarks"] || row["adj_asg_remarks"],
      placeOfLossPostcode:
        row["Place of Loss"] || row["place_of_loss_postcode"],
      recommendedReserve:
        parseFloat(row["Recmd'ed Resv"] || row["recommended_reserve"]) || 0,
      adjProviFee:
        parseFloat(row["Adj. Provi. Fee"] || row["adj_provi_fee"]) || 0,
